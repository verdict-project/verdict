...
public class ApproximateSelectQuery extends SelectQuery {
    ...
    /**
     * Verdict currently supports the aggregation queries when the outer-most query
     * includes aggregations. TODO: lift this assumption so that we also support the
     * queries with aggregations in subqueries.
     * 
     * @param queryString
     * @return
     */
    ...
    // Pair<String, AnalyticSelectStatementRewriter> rewrittenQueryAndRewriter =
    // rewriteQuery();
    //
    // String rewrittenQuery = rewrittenQueryAndRewriter.getLeft();
    // AnalyticSelectStatementRewriter rewriter =
    // rewrittenQueryAndRewriter.getRight();
    //// List<Boolean> AggregateColumnIndicator =
    // rewrittenQueryAndRewriter.getRight().getAggregateColumnIndicator();
    // Map<TableUniqueName, TableUniqueName> replacedTables =
    // rewriter.getCumulativeSampleTables();
    // Map<Integer, Integer> meanAndErrorColumnMap =
    ...
    // VerdictLogger.info(String.format("Verdict is using a sample table for %s",
    // original));
    // }
    //
    ...
    // protected Pair<String, AnalyticSelectStatementRewriter> rewriteQuery() throws
    // VerdictException {
    ...
    // 1. as soon as aggregation appears (possibly with a groupby clause), we can
    // drop the multiplicities.
    // 2. otherwise, we need to propagate the multiplicities to the outer query.
    // 3. if the multiplicity remains at the most outer query, we can drop the
    // multiplicities.
    //
    // Keysets
    // 1. If a top-level query includes a groupby; then, the groupby columns should
    // be keysets. (this is always the case).
