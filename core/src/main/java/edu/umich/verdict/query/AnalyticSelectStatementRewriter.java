...
class AnalyticSelectStatementRewriter extends SelectStatementBaseRewriter  {
	...
	public String visitSelect_list_elem(VerdictSQLParser.Select_list_elemContext ctx) {
		...
		if (ctx.getText().equals("*")) {
			...
			// This baseRewriter is only used to remember the original column name expression.
	...
	/**
	 * We have a separate visitor for derived table (rather than automatically handling using
	 *  {@link AnalyticSelectStatementRewriter#visitSubquery visitSubquery}) to access the derived table's colName2Alias
	 *  information.
	 */
	@Override
