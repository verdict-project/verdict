 ...
 *
 * <p>Impala data types:
 * https://www.cloudera.com/documentation/enterprise/5-9-x/topics/impala_datatypes.html
 *
 * @author Yongjoo Park
 */
public class JdbcResultSetMetaDataTypeForImpalaTest {
  ...
  public void testColumnTypes() throws VerdictDBDbmsException, SQLException {
    String sql =
        String.format("select * from `%s`.`%s` order by tinyintCol", IMPALA_DATABASE, TABLE_NAME);
    ...
    VerdictSingleResultFromDbmsQueryResult result =
        new VerdictSingleResultFromDbmsQueryResult(internalResult);
