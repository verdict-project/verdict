...
public class CreateTableAsSelectNodeTest {
  ...
  private static final String REDSHIFT_SCHEMA =
      "create_table_as_select_test_" + RandomStringUtils.randomAlphanumeric(8).toLowerCase();
  ...
  public static void setupH2() throws SQLException, VerdictDBException {
    ...
    h2conn =
        new JdbcConnection(
            DriverManager.getConnection(DB_CONNECTION, DB_USER, DB_PASSWORD), new H2Syntax());
  ...
  public static void setupRedshift() throws SQLException, VerdictDBDbmsException {
    ...
    redshiftVerdictConn.execute(
        String.format("DROP SCHEMA IF EXISTS \"%s\" CASCADE", REDSHIFT_SCHEMA));
    redshiftVerdictConn.execute(
        String.format("CREATE SCHEMA IF NOT EXISTS \"%s\"", REDSHIFT_SCHEMA));

    redshiftVerdictConn.execute(
        String.format(
            "CREATE TABLE \"%s\".\"%s\"(\"id\" int, \"value\" double precision)",
            REDSHIFT_SCHEMA, REDSHIFT_TEST_TABLE));
    for (int i = 0; i < 10; i++) {
      redshiftVerdictConn.execute(
          String.format(
              "INSERT INTO \"%s\".\"%s\"(\"id\", \"value\") VALUES(%s, %f)",
  ...
  public void testRedshiftCreatePartitionedTableAsSelect() throws VerdictDBDbmsException {
    ...
    CreateTableAsSelectNode node =
        new CreateTableAsSelectNode(
            ...
            SelectQuery.create(
                new AsteriskColumn(), new BaseTable(REDSHIFT_SCHEMA, REDSHIFT_TEST_TABLE)));
    ...
    List<String> partitions =
        redshiftVerdictConn.getPartitionColumns(REDSHIFT_SCHEMA, partitionedTable);
  ...
  public void testExecuteNode() throws VerdictDBException {
    ...
    ExecutableNodeBase root =
        CreateTableAsSelectNode.create(QueryExecutionPlanFactory.create("newschema"), query);
    //    ExecutionInfoToken token = new ExecutionInfoToken();
    ExecutionInfoToken newTableName = ExecutableNodeRunner.execute(h2conn, root);
  ...
  static void populateData(DbmsConnection conn, String schemaName, String tableName)
    ...
    conn.execute(
        String.format(
            "CREATE TABLE \"%s\".\"%s\"(\"id\" int, \"value\" double)", schemaName, tableName));
    for (int i = 0; i < 2; i++) {
      conn.execute(
          String.format(
              "INSERT INTO \"%s\".\"%s\"(\"id\", \"value\") VALUES(%s, %f)",
