...
public class SqlLine {
    ...
    Status initArgs(String[] args, DispatchCallback callback, ConsoleReader reader) throws IOException {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            driver = "edu.umich.verdict.jdbc.Driver";
    ...
    String getPrompt() {
        ...
        try {
            if (dbc == null) {
                return "verdict:no-db> ";
            } else {
                DatabaseMetaData meta = dbc.getDatabaseMetaData();    
                if (meta != null) {
                    try {
                        return String.format("verdict:%s> ", meta.getDatabaseProductName());
